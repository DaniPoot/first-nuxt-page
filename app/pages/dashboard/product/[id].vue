...
const handleFilesChanged = (event: Event) => {
  ...
  // Todo: create a preview view to the files
}

const removeFilePreview = (index: number) => {
  filesToUploadPreviews.value = filesToUploadPreviews.value
...
<template>
  <div class="mx-auto max-w-4xl space-y-8">
    ...
    <div v-else-if="!isCreating && !newProduct"
    ...
    <div v-if="newProduct" class="space-y-6">
      <form class="space-y-6" @submit.prevent="handleSubmit">
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
          <div class="space-y-2">
            <label class="text-sm font-medium text-gray-700 dark:text-gray-200" for="product-slug">
              Slug
            </label>
            <input id="product-slug" v-model="newProduct.slug" type="text" :class="[
            ...
            ]" placeholder="ejemplo-producto" autocomplete="off" />
          ...
          <div class="space-y-2">
            <label class="text-sm font-medium text-gray-700 dark:text-gray-200" for="product-name">
              Nombre
            </label>
            <input id="product-name" v-model="newProduct.name" type="text" :class="[
              'block w-full rounded-md bg-white px-3 py-2 shadow-sm focus:outline-none dark:bg-gray-900 dark:text-gray-100',
              fieldErrors.name
                ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                : 'border-gray-300 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700',
        ...
        <div class="space-y-2">
          <label class="text-sm font-medium text-gray-700 dark:text-gray-200" for="product-description">
            Descripción
          </label>
          <textarea id="product-description" v-model="newProduct.description" rows="4" :class="[
            'block w-full rounded-md bg-white px-3 py-2 shadow-sm focus:outline-none dark:bg-gray-900 dark:text-gray-100',
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
          <div class="space-y-4">
            <div class="space-y-2">
              <label class="text-sm font-medium text-gray-700 dark:text-gray-200" for="product-price">
                Precio (entero)
              </label>
              <input id="product-price" v-model="newProduct.price" type="number" min="0" step="1" :class="[
                'block w-full rounded-md bg-white px-3 py-2 shadow-sm focus:outline-none dark:bg-gray-900 dark:text-gray-100',
                fieldErrors.price
                  ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                  : 'border-gray-300 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700',
            ...
            <div class="space-y-2">
              <label class="text-sm font-medium text-gray-700 dark:text-gray-200" for="product-tags">
                Etiquetas
              </label>
              <input id="product-tags" v-model="newProduct.tags" type="text" :class="[
                'block w-full rounded-md bg-white px-3 py-2 shadow-sm focus:outline-none dark:bg-gray-900 dark:text-gray-100',
                fieldErrors.tagsInput
                  ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                  : 'border-gray-300 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700',
              ]" placeholder="etiqueta uno, etiqueta dos" autocomplete="off" />
          ...
          <div class="space-y-3">
            <label class="text-sm font-medium text-gray-700 dark:text-gray-200" for="product-images">
              Imágenes
            </label>
            <div v-if="(newProduct.images || []).length > 0" class="space-y-3">
              <div class="overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-800">
                <img :src="newProduct.images[selectedImageIndex]" alt="Previsualización principal del producto"
                  class="h-64 w-full object-cover" />
              </div>
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <button v-for="(image, index) in newProduct.images" :key="image + index" type="button"
                  class="overflow-hidden rounded-md border-2 transition" :class="selectedImageIndex === index
                    ? 'border-blue-500'
                    : 'border-transparent hover:border-gray-300 dark:hover:border-gray-600'
                    " @click="selectedImageIndex = index">
                  <img :src="image" :alt="`Previsualización ${index + 1}`" class="h-20 w-full object-cover"
                    loading="lazy" />
                </button>
              </div>
            </div>
            <ClientOnly>
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div v-for="(image, index) in filesToUploadPreviews" :key="image">
                  <div class="overflow-hidden rounded-lg relative">
                    <img :src="image" :alt="`Previsualización ${index + 1}`" class="h-20 w-full object-cover" />
                    <UButton color="error" icon="i-lucide-x" class="absolute top-2 right-2"
            ...
            /> -->
            <UInput v-if="!isCreating && !isSubmitting" @change="handleFilesChanged($event)" type="file" multiple
              id="product-images" rows="4" :class="[
                'block w-full rounded-md bg-white px-3 py-2 shadow-sm focus:outline-none dark:bg-gray-900 dark:text-gray-100',
                fieldErrors.imagesInput
                  ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                  : 'border-gray-300 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700',
        ...
        <div class="flex flex-wrap items-center gap-3">
          <UButton type="submit" color="primary" variant="solid" :disabled="isSubmitting" icon="i-lucide-save">
            {{ isSubmitting ? 'Guardando...' : 'Guardar producto' }}
          </UButton>
          <UButton type="button" color="neutral" variant="outline" icon="i-lucide-x" @click="handleCancel">
            Cancelar
          </UButton>
        </div>
      </form>

      <section v-if="!isCreating && product"
        ...
        <dl class="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <div class="space-y-1">
            ...
            <dd class="text-gray-900 dark:text-gray-100">
              {{ longDateTimeFormat(new Date(product?.updateAt)) }}
            </dd>
          </div>
        </dl>
      </section>
